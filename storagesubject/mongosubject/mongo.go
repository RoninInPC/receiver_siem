...
func (m MongoDB) GetType(subjectType subject.SubjectType) []subject.Subject {
	switch subjectType {
	case subject.FileT:
		files := []subject.Subject{}
		cur, _ := m.client.Database(m.Database).Collection("Files").Find(m.ctx, bson.D{})
		for cur.Next(m.ctx) {
			file := subject.File{}
			err := cur.Decode(&file)
			if err != nil {
	...
	case subject.ProcessT:
		files := []subject.Subject{}
		cur, _ := m.client.Database(m.Database).Collection("Processes").Find(m.ctx, bson.D{})
		for cur.Next(m.ctx) {
			file := subject.Process{}
			err := cur.Decode(&file)
			if err != nil {
	...
	case subject.UserT:
		files := []subject.Subject{}
		cur, _ := m.client.Database(m.Database).Collection("Users").Find(m.ctx, bson.D{})
		for cur.Next(m.ctx) {
			file := subject.User{}
			err := cur.Decode(&file)
			if err != nil {
	...
	case subject.PortTablesT:
		files := []subject.Subject{}
		cur, _ := m.client.Database(m.Database).Collection("Ports").Find(m.ctx, bson.D{})
		for cur.Next(m.ctx) {
			file := subject.User{}
			err := cur.Decode(&file)
			if err != nil {
...
func (m MongoDB) GetHosts() []string {
	lists, _ := m.client.ListDatabases(m.ctx, bson.D{})
	answer := []string{}
	for _, base := range lists.Databases {
		answer = append(answer, base.Name)
	}
	return answer
}
